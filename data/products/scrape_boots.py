#!/usr/bin/env python3
"""
Boots.com skincare scraper — run once to populate Cloud SQL.
...
DELAY = 3  # seconds between ScraperAPI calls
...
def scraper_get(url: str, render: bool = True, premium: bool = False,
                wait_ms: int = 0, session: int = 1, js_snippet: str = "") -> requests.Response | None:
    params = {
    ...
    try:
        resp = requests.get("http://api.scraperapi.com", params=params, timeout=180)
        if resp.status_code == 200:
            return resp
        log.warning(f"ScraperAPI returned {resp.status_code} for {url}")
        return None
    except Exception as e:
        log.error(f"ScraperAPI error for {url}: {e}")
        return None


# Clicks "View more" up to N times, waiting 3s after each click for AJAX to settle
def _load_more_js(clicks: int = 10) -> str:
    return f"""
(async () => {{
    for (let i = 0; i < {clicks}; i++) {{
        const btn = document.querySelector('[aria-label*="more products"], .oct-listers__pagination-bottom__button');
        if (!btn) break;
        btn.click();
...
def extract_product_urls(html: str) -> list[str]:
    ...
    # Boots product URLs end with a 7-8 digit numeric ID, e.g.:
    # /cerave-moisturising-cream-454ml-10302107
...
def get_page_count(html: str) -> int:
    ...
    # Fallback: look for last page number in pagination links
    page_nums = [
...
def parse_product_page(html: str, url: str, category: str = "") -> dict | None:
    ...
    # Brand fallback: extract from URL slug, stopping before product descriptor words
    ...
    # Derive image from boots_id — all Boots images follow this Scene7 pattern
    ...
    # Use Boots product ID (from URL) as a stable dedup key
...
def _parse_selectors(soup: BeautifulSoup) -> dict | None:
    ...
    # Last resort: extract from <title> tag ("Product Name - Boots")
    if not name:
...
def _ingredients_from_obf(product_name: str) -> str:
    """Search Open Beauty Facts by product name and return the first match's ingredients."""
    try:
        resp = requests.get(
        ...
        if products:
            return (products[0].get("ingredients_text") or "").strip()
    except Exception:
        pass
    return ""
...
def _extract_ingredients(soup: BeautifulSoup) -> str:
    # Strategy 1: Boots-specific — <h3 id="product_ingredients"> followed by <p>
    ...
    # Strategy 2: any element whose id/class contains "ingredient"
    ...
    # Strategy 3: heading text then next sibling
...
def _brand_from_url(url: str) -> str:
    ...
    # Try lookup with increasing prefix length (1-4 words)
    words = slug.split("-")
    ...
    # Fallback: first word before a product descriptor
    brand_words = []
    for word in words:
...
def _extract_boots_id(url: str) -> str:
    # Boots URLs end with -NNNNNNN (7-8 digit ID), e.g. /cerave-moisturising-cream-10302107
...
def scrape_category(category_name: str, path: str, conn, session: int = 1, limit: int | None = None, skip: int = 0) -> int:
    base_url = BOOTS_BASE + path
    log.info(f"\n{'='*60}")
    log.info(f"Category: {category_name}  →  {base_url}")

    # Try sitemap first (no JS, more reliable)
    product_urls = set(get_urls_from_sitemap())

    # Fall back to category page — click "View more" up to 10 times to load all products
    if not product_urls:
        log.info("Sitemap failed — scraping category page with View More clicks")
        resp = scraper_get(
            base_url, render=True, wait_ms=5000,
            session=session, js_snippet=_load_more_js(clicks=10)
        )
        if not resp:
            log.error(f"Failed to fetch category page, skipping {category_name}")
            return 0

        product_urls = set(extract_product_urls(resp.text))
        log.info(f"Found {len(product_urls)} product URLs after View More clicks")

    product_list = list(product_urls)
    if skip:
        product_list = product_list[skip:]
        log.info(f"Skipping first {skip} URLs (--skip)")
    if limit:
        product_list = product_list[:limit]
    log.info(f"Total product URLs: {len(product_urls)}" + (f" (skipping {skip})" if skip else "") + (f" (capped at {limit})" if limit else ""))
    ...
    for i, url in enumerate(product_list, 1):
        ...
        if already_scraped(boots_id, conn):
            log.info(f"  [{i}/{len(product_urls)}] SKIP (already in DB): {url}")
            continue

        log.info(f"  [{i}/{len(product_urls)}] Scraping: {url}")
        time.sleep(DELAY)

        resp = scraper_get(url, render=True, wait_ms=6000)
        if not resp:
            continue
...
def main(limit: int | None = None, skip: int = 0):
    log.info("Boots scraper starting")
    ...
    total = 0
    for i, (category_name, path) in enumerate(CATEGORIES, 1):
        saved, conn = scrape_category(category_name, path, conn, session=i, limit=limit, skip=skip)
        total += saved
...
def test_one():
    ...
    log.info("TEST MODE — scraping one product from Moisturiser category")
    ...
    # Fall back to category page with View More clicks
    if not urls:
        log.info(f"Fetching category page with View More clicks: {base_url}")
        resp = scraper_get(base_url, render=True, wait_ms=5000, js_snippet=_load_more_js(clicks=3))
        if resp:
    ...
    if not urls:
        ...
        if all_hrefs:
            ...
            print(resp.text[2000:5000])  # skip <head>, show <body> area
        return
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        # --limit N  cap how many products to scrape in this run
        # --skip N   skip the first N URLs (resume after a crash)
        limit = None
