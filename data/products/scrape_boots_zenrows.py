#!/usr/bin/env python3
"""
Boots.com skincare scraper (ZenRows) — run once to populate Cloud SQL.

Usage:
    cd skincare_advisor
    python data/products/scrape_boots_zenrows.py
...
DELAY = 3  # seconds between ZenRows calls
...
def zenrows_get(url: str, render: bool = True) -> requests.Response | None:
    """Fetch a URL via ZenRows API.

    Terminates the process immediately on 401 (invalid/missing API key).
    ...
    try:
        ...
        if resp.status_code == 401:
            log.error("ZenRows returned 401 Unauthorized — check ZENROWS_API_KEY. Terminating.")
            sys.exit(1)
        if resp.status_code == 200:
            return resp
        log.warning(f"ZenRows returned {resp.status_code} for {url}: {resp.text[:200]}")
        return None
    except Exception as e:
        log.error(f"ZenRows error for {url}: {e}")
        return None
...
def scrape_category(category_name: str, path: str, conn, session: int = 1, limit: int | None = None, skip: int = 0) -> tuple[int, object]:
    ...
    if not product_urls:
        log.info("Sitemap failed — scraping category page via ZenRows")
        resp = zenrows_get(base_url, render=True)
        if not resp:
    ...
    for i, url in enumerate(product_list, 1):
        ...
        resp = zenrows_get(url, render=True)
        if not resp:
            continue
...
def main(limit: int | None = None, skip: int = 0):
    log.info("Boots scraper (ZenRows) starting")
