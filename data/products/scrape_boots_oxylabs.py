#!/usr/bin/env python3
"""
Boots.com skincare scraper (Oxylabs) — run once to populate Cloud SQL.

Usage:
    cd skincare_advisor
    python data/products/scrape_boots_oxylabs.py

Scrapes face/skincare categories, visits each product page for ingredients,
and saves to skincare_advisor.products.

Requires env vars:
    OXYLABS_USERNAME  — Oxylabs account username (e.g. skincare_tcOEG)
...
DELAY = 3  # seconds between Oxylabs calls
...
def oxylabs_get(url: str, render: bool = True) -> requests.Response | None:
    """Fetch a URL via Oxylabs realtime API, returning a Response-like object
    whose .text/.status_code match what the rest of the scraper expects."""
    payload = {
        "source": "universal",
        "url": url,
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if results and results[0].get("content"):
                # Wrap the HTML content in a lightweight object so callers can
                # use .text and .status_code exactly as they do with requests.Response
                return _OxylabsResponse(results[0]["content"], results[0].get("status_code", 200))
            log.warning(f"Oxylabs returned no content for {url}: {data}")
            return None
        log.warning(f"Oxylabs HTTP {resp.status_code} for {url}: {resp.text[:200]}")
        return None
    except Exception as e:
        log.error(f"Oxylabs error for {url}: {e}")
        return None


class _OxylabsResponse:
    """Minimal response shim — exposes .text and .status_code like requests.Response."""
...
def scrape_category(category_name: str, path: str, conn, session: int = 1, limit: int | None = None, skip: int = 0) -> int:
    ...
    # Fall back to category page
    if not product_urls:
        log.info("Sitemap failed — scraping category page via Oxylabs")
        resp = oxylabs_get(base_url, render=True)
        if not resp:
            log.error(f"Failed to fetch category page, skipping {category_name}")
            return 0

        product_urls = set(extract_product_urls(resp.text))
        log.info(f"Found {len(product_urls)} product URLs from category page")
    ...
    for i, url in enumerate(product_list, 1):
        ...
        resp = oxylabs_get(url, render=True)
        if not resp:
            continue
...
def main(limit: int | None = None, skip: int = 0):
    log.info("Boots scraper (Oxylabs) starting")
